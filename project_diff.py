...
Original file is located at
    https://colab.research.google.com/drive/11ayrLZ6bj2gBRwheMdog6GNrQvCvoDt8
"""
...
essential_columns = ['Group', 'SubjectID', 'Run']
...
X = animal_data.drop(columns=['Group', 'Levels', 'SubjectID', 'Run'])
y = animal_data['Group']
...
6. **Neural Network (MLP Classifier)**:
   - **Motivation**: A neural network was chosen for its flexibility and capability in capturing complex, non-linear relationships in high-dimensional data. While models like LDA and Logistic Regression focus on linear separation, neural networks can represent a wider range of patterns, making them suitable for tasks where intricate interactions between ABR metrics might influence classification. The MLP Classifier, a type of neural network, provides an adaptable structure that can model subtle patterns in the data without manually specifying feature interactions, which aligns well with the goal of leveraging ABR metrics to differentiate between groups
   - **Training**: The MLP Classifier is a feedforward neural network consisting of multiple layers of interconnected nodes. Each node applies a non-linear activation function, allowing the network to learn complex decision boundaries. For this project, various configurations of hidden layers and activation functions were explored, including rectified linear units (ReLU) and hyperbolic tangent (tanh) functions, to ensure flexibility in capturing non-linear patterns. Regularization was applied through the alpha parameter, which penalizes overly complex solutions to prevent overfitting. Cross-validation was used to optimize hyperparameters such as the number of neurons in each layer, learning rate, activation functions, and the strength of regularization, ensuring that the model generalized well to unseen data. During training, backpropagation and gradient descent were used to adjust the network's weights, iteratively reducing the error in predictions by minimizing the loss function over each epoch.
   - **Evaluation**: **Accuracy** measures the overall performance of the classifier, while ROC-AUC is used to assess the network’s ability to distinguish between classes, especially useful for understanding its performance on binary comparisons (e.g., control vs. 96 dB). The F1 Score provides a balanced metric for scenarios with class imbalance. For interpretability, feature importance was analyzed through permutation importance, which evaluates how each ABR metric influences predictions by shuffling individual features and measuring the effect on model performance. This process highlights the ABR metrics that the neural network relied on most for classification, offering insight into how each metric contributes to distinguishing groups.

#### Summary of Evaluation Metrics
- **Accuracy**: This is the primary metric used to measure the overall classification performance across models, indicating the proportion of correct predictions. A higher accuracy score is desirable as it shows the model's capability to correctly classify most instances. Accuracy is particularly useful for providing a general sense of model performance but may not fully capture nuances in imbalanced datasets.
- **ROC-AUC Score**: The ROC-AUC score is used to evaluate the model's discrimination ability, especially in binary comparisons (e.g., control vs. 96 dB groups). A higher ROC-AUC score indicates better ability to distinguish between classes. This metric is useful for assessing models where classification boundaries may not be clear-cut, providing insight into how well the model separates positive from negative classes.
- **F1 Score**: The F1 Score, which is the harmonic mean of precision and recall, is valuable in cases of class imbalance. By balancing the trade-off between precision (true positive rate) and recall (sensitivity), the F1 Score ensures that the model performs well in capturing both relevant cases and avoiding false positives, which is particularly important in the context of ABR metrics where certain classes might be underrepresented.
- **Feature Importance and Interpretability**: Interpretability is a key metric in this project, as identifying the most relevant ABR metrics helps in understanding how these features contribute to distinguishing noise exposure groups. Feature importance, coefficients, and latent structures derived from models like Random Forest, Logistic Regression, and LDA provide insights into the role of individual metrics. This interpretability adds value to the analysis by allowing stakeholders to pinpoint specific ABR metrics that significantly impact classification, enhancing the model’s practical applicability.
...
feature_importances_mlp_subset = feature_importances.importances_mean
feature_importance_df = pd.DataFrame({
    'Feature': X_test_subset.columns,
...
results = pd.DataFrame(xgb_grid_subset.cv_results_)
filtered_results = results[(results['param_max_depth'] == 3) & (results['param_learning_rate'] == 0.2)]

plt.figure(figsize=(10, 6))
plt.plot(filtered_results['param_n_estimators'], filtered_results['mean_test_score'], marker='o')
plt.xlabel("Number of Estimators (n_estimators)")
plt.ylabel("Mean Test Accuracy")
plt.title("XGBoost: Effect of n_estimators on Accuracy (Control and 96 dB Groups at T2)")
plt.grid(True)
plt.show()
...
create_heatmap(importance_df_entire_scaled, "Feature Importance Across Models with Complete Dataset")
...
create_heatmap(importance_df_subset_scaled, "Variable Importance Across Models with Subset of Control vs 96db Groups at T2")